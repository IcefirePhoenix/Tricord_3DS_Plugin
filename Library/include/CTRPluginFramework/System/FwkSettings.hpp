...
namespace CTRPluginFramework
{
    ...
    struct FwkSettings
    {
        ...
        // Controller options
        bool UseGameHidMemory; ///< Set to true to prevent using hidInit and use the game hid memory and capabilities (only change it during plugin init). | Default: false
        bool AreN3DSButtonsAvailable; ///< Set to false to signal if the N3DS buttons are available (only change it during plugin init). | Default: true
